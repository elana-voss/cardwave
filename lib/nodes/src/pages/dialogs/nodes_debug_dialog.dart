...
/// open chat: scene, goal/phase, every per-character emotion (with
/// lockout countdown), every node in the pool (with runtime
/// countdowns + predicate source), and the tail of the event log.
///
/// Per spec §10 this is a development and tuning tool — not surfaced
/// for end users to interact with. Snapshot only; close and re-open
/// to see updates.
class NodesDebugDialog extends StatelessWidget {
  ...
  /// Tail size for the event log section — older entries are dropped
  /// from the display so a long session doesn't blow up the dialog.
            ...
            _Section(
              ...
              child: pool.active.isEmpty
                  ...
                  : Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        for (final node in pool.active) _NodeBlock(node: node),
                      ],
                    ),
            ),
            _Section(
      ...
      child: Column(
        ...
        children: [
          ...
          for (final entry in character.emotion.entries)
            Text(
              '  ${entry.key.name.padRight(13)} '
              '${entry.value.value.toStringAsFixed(2)}'
              '${entry.value.lockoutTurnsRemaining > 0 ? "  (lock ${entry.value.lockoutTurnsRemaining})" : ""}',
            ),
        ],
      ),
    );
...
class _NodeBlock extends StatelessWidget {
  const _NodeBlock({required this.node});
