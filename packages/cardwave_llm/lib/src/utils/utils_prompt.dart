...
/// Placeholder substitution engine used by the LLM module's prompt
/// builders. Mirrors the app-side `UtilsPrompt` so the module can resolve
/// `{{char}}`, `{{user}}`, `{{getvar:X}}`, `{{$globalvar}}`, etc., without
/// importing app code. Kept in lockstep with `lib/common/src/utils/utils_prompt.dart`
/// — divergence breaks user-visible macro behaviour in package-built prompts.
class UtilsPrompt {
