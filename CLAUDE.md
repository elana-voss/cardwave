...
- **An explicit instruction is the spec: execute it verbatim — don't narrow it, don't pad it.** When the user says "run ALL X, including Y," run every X and the named Y, including the costly or opt-in ones — their explicit word already grants the cost, so don't re-ask about it and don't drop items on a cost/safety default of your own. Equally, don't pull in adjacent items they didn't name. Substituting your own judgment for a directive they already stated is the divergence.
...
- **For a "run it" request, use the simplest proven invocation — don't wrap it in custom orchestration.** When a documented command or existing script already runs the thing (e.g. a suite command in a README), use that. A bespoke runner with its own logging, monitor, and retry is more surface to break, and that machinery — not the task itself — is usually what fails.
...
- **To run the whole suite, use the proven runner — never hand-roll one.** `../cardwave-dev/scripts-dev/run_all_integration_tests.sh android` is the built runner: it runs every file one at a time, caps each at 240s, and prints a pass/fail tally. It calls `run_integration_test.sh`, which sources both API keys from `../cardwave-dev/secrets/test-secrets.env`. This is how the suite is meant to run.
  - Do **not** invent your own runner with reboots/monitors, and do **not** run one giant `flutter test integration_test/` over the whole folder — emulator state leaks between tests and a mid-run failure can't be pinned to a file.
  - Do **not** kill a `flutter test` while it is on `Installing …-debug.apk`. Killing mid-install wedges the emulator's package manager (later installs hang, `pm` stops responding), which then looks like an emulator bug. Let a run finish, or let its per-test timeout move on.
  - The `Pixel_6` emulator needs **6 GB RAM** (`hw.ramSize=6144` in `~/.android/avd/Pixel_6.avd/config.ini`, per-app heap `vm.heapSize=512`). At the old 2 GB, the package-installer service ran out of memory after only three or four full APK installs and died with `Failure calling service package: Broken pipe (32)` — every install after that failed and tests could not launch, which reads like the emulator crashing but is really out-of-memory. Each test file is a separate full APK, so a whole-suite run does 50-plus back-to-back installs and will exhaust a small emulator. A RAM change only takes effect on a **cold restart** — fully close and relaunch the emulator; a soft reboot keeps the old size.
  - The runner does not pass `RUN_VIDEO`, so the paid video test skips by default; run that one file separately when it's wanted. It does pass the NanoGPT key from the secrets file, so a dead NanoGPT key makes the two two-provider tests fail instead of skip.
  - A Gradle `compressDebugAssets` failure on `kernel_blob.bin` ("Could not … kernel_blob.bin") is a build-environment lock, **not** a test or app failure — the app never built, so the result is meaningless. It appears intermittently on this Windows machine; the likely cause is a second Gradle builder on the same `build/` folder (e.g. Android Studio open on the project), so prefer only one builder active during a run. Re-running the file usually builds cleanly. Do not report these as failing tests.
  - When files fail, re-run only those files — never relaunch the whole suite from the start. Completed passes are results, not work to redo.
- **The two `local_provider_*` tests default to an in-process mock server, so they pass in the normal suite with no extra setup.** To exercise them against a *real* OpenAI-compatible server, use the purpose-built runner `../cardwave-dev/scripts-dev/run_kobold_integration.sh`, which launches KoboldCpp and sets `LOCAL_LLM_BASE_URL=http://10.0.2.2:PORT/v1` correctly (`10.0.2.2` is the emulator's route to the host; plain `localhost` on the emulator points at the emulator itself). Setting `LOCAL_LLM_BASE_URL` is what flips the test off the mock and onto that URL. If a server is already up on the host, the app's "Server URL" field defaults to `http://localhost:5001/v1`, so `adb reverse tcp:5001 tcp:5001` also lets the emulator reach it.
